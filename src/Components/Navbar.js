...
const history = useNavigate();
  ...
  const handleMobileMenuOpen = (event) => {
    ...
    const handleExplore = () => {
        let win = window.open('https://about.instagram.com/features/reels', '_blank');
        win.focus();
      ...
      <AppBar position="fixed" sx={{background:'white'}}>
        <Toolbar>
            <div style={{marginLeft:'5%'}}>
                      <img src={insta} onClick={bannerClick} style={{ width: '20vh' }} />
            </div>
          <Box sx={{ flexGrow: 1 }} />
                  <Box sx={{ display: { xs: 'none', md: 'flex' }, color: 'black', alignItems: 'center', marginRight:'2rem'}}>
                 <HomeIcon onClick={bannerClick} sx={{marginRight:'1.5rem',cursor:'pointer'}}></HomeIcon> 
